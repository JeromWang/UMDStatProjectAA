...
def load_data():
	...
	with open('tweets', 'r') as f:
		for line in f:
			docs_train.append(' '.join(list(line)))
	...
	category_num = len(set(labels_train))
	labels_train = np_utils.to_categorical(labels_train, category_num)

	vocab_size = len(Counter(' '.join(docs_train)))
	print(Counter(' '.join(docs_train)))

	print (docs_train[0])
	embedded_docs = [one_hot(doc, vocab_size) for doc in docs_train]
	...
	return padded_docs, labels_train, max_length, vocab_size, category_num
...
checkpoint = ModelCheckpoint('AA-C.hdf5', monitor='val_acc', verbose=1, save_best_only=True, mode='auto')
